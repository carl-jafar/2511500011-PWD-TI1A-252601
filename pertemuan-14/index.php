  ...
  <!-- header -->
    ...
    <!-- about -->
        <section id="about">
            <h2>Tentang Saya</h2>
            <?php
    // Ambil data biodata TERBARU dari database
    $sqlBio = "SELECT * FROM tbl_about_me ORDER BY id DESC LIMIT 1";
    $qBio = mysqli_query($conn, $sqlBio);
    $dbmahasiswa = mysqli_fetch_assoc($qBio);

    if ($dbmahasiswa) {
        // Mapping kolom database ke array agar sesuai dengan $fieldConfig
        $arrBiodata = [
            "nim" => $dbmahasiswa["nim"],
            "nama" => $dbmahasiswa["nama_lengkap"],
            "tempat" => $dbmahasiswa["tempat_lahir"],
            "tanggal" => $dbmahasiswa["tanggal_lahir"],
            "hobi" => $dbmahasiswa["hobi"],
            "pasangan" => $dbmahasiswa["pasangan"],
            "pekerjaan" => $dbmahasiswa["pekerjaan"],
            "ortu" => $dbmahasiswa["nama_ortu"],
            "kakak" => $dbmahasiswa["nama_kakak"],
            "adik" => $dbmahasiswa["nama_adik"]
        ];
        echo tampilkanBiodata($fieldConfig, $arrBiodata);
    } else {
        echo "<p>Biodata belum tersedia di database.</p>";
    }     ?>
        </section>
