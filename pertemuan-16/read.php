<?php
  session_start();
  require 'koneksi.php';
  require 'fungsi.php';

  // Query 1: Ambil data pesan tamu
  $sql = "SELECT * FROM tbl_tamu ORDER BY cid DESC";
  $q = mysqli_query($conn, $sql);

  // Query 2: Ambil data biodata mahasiswa
  $sqlBio = "SELECT * FROM tbl_biodata_dsn ORDER BY id DESC";
  $qBio = mysqli_query($conn, $sqlBio);

  // Validasi jika query gagal
  if (!$q || !$qBio) {
    die("Query error: " . mysqli_error($conn));
  }
?>

<?php
  $flash_sukses = $_SESSION['flash_sukses'] ?? ''; 
  $flash_error  = $_SESSION['flash_error'] ?? ''; 
  unset($_SESSION['flash_sukses'], $_SESSION['flash_error']); 
?>

<?php if (!empty($flash_sukses)): ?>
<div style="padding:10px; margin-bottom:10px; background:#d4edda; color:#155724; border-radius:6px;">
    <?= $flash_sukses; ?>
</div>
<?php endif; ?>

<?php if (!empty($flash_error)): ?>
<div style="padding:10px; margin-bottom:10px; background:#f8d7da; color:#721c24; border-radius:6px;">
    <?= $flash_error; ?>
</div>
<?php endif; ?>

<h3>Daftar Pesan Tamu</h3>
<table border="1" cellpadding="8" cellspacing="0" style="margin-bottom: 30px;">
    <tr>
        <th>No</th>
        <th>Aksi</th>
        <th>ID</th>
        <th>Nama</th>
        <th>Email</th>
        <th>Pesan</th>
        <th>Created At</th>
    </tr>
    <?php $i = 1; ?>
    <?php while ($row = mysqli_fetch_assoc($q)): ?>
    <tr>
        <td><?= $i++ ?></td>
        <td>
            <a href="edit.php?cid=<?= (int)$row['cid']; ?>">Edit</a>
            <a onclick="return confirm('Hapus <?= htmlspecialchars($row['cnama']); ?> ?')"
                href="proses_delete.php?cid=<?= (int)$row['cid']; ?>">Delete</a>
        </td>
        <td><?= $row['cid']; ?></td>
        <td><?= htmlspecialchars($row['cnama']); ?></td>
        <td><?= htmlspecialchars($row['cemail']); ?></td>
        <td><?= nl2br(htmlspecialchars($row['cpesan'])); ?></td>
        <td><?= formatTanggal(htmlspecialchars($row['dcreated_at'])); ?></td>
    </tr>
    <?php endwhile; ?>
</table>

<hr>

<h3>Daftar Biodata Dosen</h3>
<table border="1" cellpadding="8" cellspacing="0">
    <tr>
        <th>Kode Dosen</th>
        <th>Nama Dosen</th>
        <th>Alamat Rumah</th>
        <th>Nama Lengkap</th>
        <th>Tanggal Jadi Dosen</th>
        <th>Homebase Prodi</th>
        <th>Nomor HP</th>
        <th>Nama Pasangan</th>
        <th>Nama Anak</th>
        <th>Created At</th>
    </tr>
    <?php $j = 1; ?>
    <?php while ($rowBio = mysqli_fetch_assoc($qBio)): ?>
    <tr>
        <td><?= $j++ ?></td>
        <td>
            <a href="edit_biodata.php?id=<?= (int)$rowBio['id']; ?>">Edit</a>
            <a onclick="return confirm('Hapus biodata <?= htmlspecialchars($rowBio['Nama_Dosen']); ?>?')" 
               href="proses_delete_bio.php?id=<?= (int)$rowBio['id']; ?>">Delete</a>
        </td>
        <td><?= htmlspecialchars($rowBio['kode dosen']); ?></td>
        <td><?= htmlspecialchars($rowBio['Nama Dosen']); ?></td>
        <td><?= htmlspecialchars($rowBio['Alamat Rumah']); ?></td>
        <td><?= htmlspecialchars($rowBio['Tanggal Jadi Dosen']); ?></td>
        <td><?= htmlspecialchars($rowBio['Homebase Prodi']); ?></td>
        <td><?= htmlspecialchars($rowBio['Nomor HP']); ?></td>
        <td><?= htmlspecialchars($rowBio['Nama Pasangan']); ?></td>
        <td><?= htmlspecialchars($rowBio['Nama Anak']); ?></td>
        <td><?= formatTanggal($rowBio['dcreated_at']); ?></td>
    </tr>
    <?php endwhile; ?>
</table>